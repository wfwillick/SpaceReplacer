...
$folders | ForEach-Object -Process {
    ...
    For($i = 1; $i -le $folders.count; $i++)
            {Write-Progress -Activity "Renaming Folders" -Status "Renaming folder $i" -PercentComplete ($i/$folders.count * 100)} $folders | Select Name

    #Joins the parent path with the now renamed folder.
...
$files | ForEach-Object -Process {
    ...
    For($i = 1; $i -le $files.count; $i++)
            {Write-Progress -Activity "Renaming Files" -Status "Renaming file $i" -PercentComplete ($i/$files.count * 100)} $files | Select Name
